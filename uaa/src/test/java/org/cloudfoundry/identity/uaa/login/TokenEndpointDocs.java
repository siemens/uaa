...
public class TokenEndpointDocs extends AbstractTokenMockMvcTests {
    ...
    private final ParameterDescriptor codeVerifier = parameterWithName(PkceValidationService.CODE_VERIFIER).description("<small><mark>UAA 4.22.0</mark></small> [PKCE](https://tools.ietf.org/html/rfc7636) Code Verifier. A `code_verifier` parameter must be provided if a `code_challenge` parameter was present in the previous call to `/oauth/authorize`. The `code_verifier` must match the used `code_challenge`").attributes(key("constraints").value("Optional"), key("type").value(STRING));
